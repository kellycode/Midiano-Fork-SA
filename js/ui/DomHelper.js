...
export class DomHelper {
    ...
    static createSliderWithLabelAndField(id, label, val, min, max, step, onChange) {
        let displayDiv = DomHelper.createElement("div", {}, { id: id + "Field", className: "sliderVal", innerHTML: val });
        ...
        let cont = DomHelper.createElement("div", {}, { id: id + "container", className: "sliderContainer" });
        let labelDiv = DomHelper.createElement("label", {}, { id: id + "label", className: "sliderLabel", innerHTML: label });
        let slider = DomHelper.createSlider(id, val, min, max, step, onChangeInternal);
        cont.appendChild(labelDiv);
    ...
    static createCheckbox(text, onChange, value, isChecked) {
        ...
        let label = DomHelper.createElementWithClass("checkboxlabel", "label", {}, { innerHTML: text, for: id });
    ...
    static createFileInput(text, callback, id) {
        ...
        let inp = DomHelper.createElement("input", { display: "none" }, { id: id, type: "file" });
    ...
    static createInputSelect(title, items, callback) {
        ...
        let label = DomHelper.createElementWithClass("inputSelectLabel", "label", {}, { innerHTML: title });
        selectBox.appendChild(label);
        let selectTag = DomHelper.createElementWithIdAndClass(title, "inputSelect", "select");
    ...
    static createColorPickerGlyphiconText(glyph, text, startColor, onChange) {
        ...
        let glyphBut = DomHelper.createGlyphiconTextButton("colorPickerGlyph" + glyph + replaceAllString(text, " ", "_"), glyph, text, () => {
            pickrEl.show();
        });
        ...
        let getGlyphEl = () => glyphBut.querySelector("#colorPickerGlyph" + glyph + replaceAllString(text, " ", "_") + " .glyphicon");
    ...
    static createColorPickerText(text, startColor, onChange) {
        ...
        let label = DomHelper.createDivWithClass("colorLabel settingLabel", {}, { innerHTML: text });

        let colorButtonContainer = DomHelper.createDivWithClass("colorPickerButtonContainer");
        ...
        colorPicker.on("change", (color) => {
            colorButtonContainer.style.backgroundColor = colorPicker.getColor().toRGBA().toString();
