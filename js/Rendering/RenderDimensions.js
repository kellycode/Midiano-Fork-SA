...
export class RenderDimensions {
    ...
    getAbsolutePianoPosition() {
        return this.windowHeight - this.whiteKeyHeight;
    }
     ...
     */
    getKeyX(noteNumber) {
        return (this.getWhiteKeyNumber(noteNumber) - this.getWhiteKeyNumber(this.minNoteNumber)) * this.whiteKeyWidth + (this.whiteKeyWidth - this.blackKeyWidth / 2) * isBlack(noteNumber);
    }
    ...
    getYForTime(time) {
        ...
        if (getSetting("reverseNoteDirection")) {
            ...
            return height - (time / noteToHeightConst) * height - (height - this.getAbsolutePianoPosition());
        }
    }
	 ...
	 */
    getNoteDimensions(noteNumber, currentTime, noteStartTime, noteEndTime, sustainOffTime) {
        ...
        if (sustainOffTime > noteEndTime) {
            sustainH = ((sustainOffTime - noteEndTime) / this.getNoteToHeightConst()) * (this.windowHeight - this.whiteKeyHeight);
    ...
    getMilisecondsDisplayedAfter() {
        let pianoPos = getSetting("pianoPosition") / 100;
        return pianoPos * (this.getNoteToHeightConst() / getSetting("playedNoteFalloffSpeed"));
    }
