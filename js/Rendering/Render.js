...
export class Render {
    ...
    render(playerState) {
        ...
        if (this.showKeyNamesOnPianoWhite != getSetting("showKeyNamesOnPianoWhite") || this.showKeyNamesOnPianoBlack != getSetting("showKeyNamesOnPianoBlack")) {
            this.showKeyNamesOnPianoWhite = getSetting("showKeyNamesOnPianoWhite");
            this.showKeyNamesOnPianoBlack = getSetting("showKeyNamesOnPianoBlack");
            this.pianoRender.resize();
        }

        if (this.renderDimensions.pianoPositionY != parseInt(getSetting("pianoPosition"))) {
            this.renderDimensions.pianoPositionY = parseInt(getSetting("pianoPosition"));
        ...
        if (!playerState.loading && playerState.song) {
            ...
            this.sustainRender.render(time, playerState.song.sustainsBySecond, playerState.song.sustainPeriods);
        ...
        this.debugRender.render(renderInfosByTrackMap, this.mouseX, this.mouseY, this.renderDimensions.menuHeight);
                        ...
                        for (let i = firstSecondShown; i < lastSecondShown; i++) {
                            if (track.notesBySeconds[i]) {
                                track.notesBySeconds[i]
                                    ...
                                    .forEach((renderInfo) => (renderInfo.isBlack ? renderInfoByTrackMap[trackIndex].black.push(renderInfo) : renderInfoByTrackMap[trackIndex].white.push(renderInfo)));
                            }
                        }
                    }
                });
    ...
    getNoteRenderInfo(note, time) {
        time *= 1000;
        let noteDims = this.renderDimensions.getNoteDimensions(note.noteNumber, time, note.timestamp, note.offTime, note.sustainOffTime);
        ...
        return {
            ...
            fillStyle: note.fillStyle ? note.fillStyle : isKeyBlack ? getTrackColor(note.track).black : getTrackColor(note.track).white,
            currentTime: time,
            isBlack: isKeyBlack,
            noteDims: noteDims,
            isOn: isOn,
    ...
    drawBPM(playerState) {
        ...
        this.ctx.fillText(Math.round(playerState.bpm) + " BPM", 20, this.renderDimensions.menuHeight + PROGRESS_BAR_CANVAS_HEIGHT + 12);
    }
    ...
    setupCanvases() {
        ...
        DomHelper.setCanvasSize(this.getMainCanvas(), this.renderDimensions.windowWidth, this.renderDimensions.windowHeight);

        DomHelper.setCanvasSize(this.getProgressBarCanvas(), this.renderDimensions.windowWidth, 20);

        DomHelper.setCanvasSize(this.getForegroundCanvas(), this.renderDimensions.windowWidth, this.renderDimensions.windowHeight);
        this.setCtxBlur();
    }
    getBgCanvas() {
        if (!this.cnvBG) {
            ...
            this.cnvBG.id = "BackgroundCanvas";
    ...
    getForegroundCanvas() {
        if (!this.cnvForeground) {
            this.cnvForeground = DomHelper.createCanvas(this.renderDimensions.windowWidth, this.renderDimensions.windowHeight, {
                position: "absolute",
                top: "0px",
                left: "0px",
            });
            this.cnvForeground.id = "ForegroundCanvas";
    ...
    getProgressBarCanvas() {
        if (!this.progressBarCanvas) {
            this.progressBarCanvas = DomHelper.createCanvas(this.renderDimensions.windowWidth, PROGRESS_BAR_CANVAS_HEIGHT, {});
    ...
    isOnMainCanvas(position) {
        return (
            (position.x > this.renderDimensions.menuHeight && position.y < this.renderDimensions.getAbsolutePianoPosition()) ||
            position.y > this.renderDimensions.getAbsolutePianoPosition() + this.renderDimensions.whiteKeyHeight
        );
    }
    setMouseCoords(x, y) {
        this.mouseX = x;
