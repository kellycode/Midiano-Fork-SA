...
export class NoteRender {
    ...
    getActiveNotes(notesRenderInfoBlack, notesRenderInfoWhite) {
        ...
        let activeNotesWhite = notesRenderInfoWhite.slice(0).filter((renderInfo) => renderInfo.isOn);
    ...
    createNoteParticles(activeNotes, colWhite, colBlack) {
        if (getSetting("showParticlesTop") || getSetting("showParticlesBottom")) {
            activeNotes.white.forEach((noteRenderInfo) => this.createNoteParticle(noteRenderInfo));
            activeNotes.black.forEach((noteRenderInfo) => this.createNoteParticle(noteRenderInfo));
        }
    }
