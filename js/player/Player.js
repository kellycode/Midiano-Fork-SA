...
class Player {
    ...
    handleScroll(stacksize) {
        if (this.scrolling != 0) {
            ...
            this.scrolling = (Math.abs(this.scrolling) - Math.max(Math.abs(this.scrolling * 0.003), this.playbackSpeed * 0.001)) * (Math.abs(this.scrolling) / this.scrolling) || 0;
    ...
    playTick() {
        ...
        while (this.isNextNoteReached(currentTime)) {
            ...
            if (this.noteSequence[0] && (!isTrackRequiredToPlay(this.noteSequence[0].track) || this.isInputKeyPressed(this.noteSequence[0].noteNumber))) {
            ...
            if (beatsBySecond[second]) {
                beatsBySecond[second].forEach((beatTimestamp) => {
                    if (!this.playedBeats.hasOwnProperty(beatTimestamp) && beatTimestamp / 1000 < currentTime + 0.5) {
                        let newMeasure = getCurrentSong().measureLines[Math.floor(beatTimestamp / 1000)] && getCurrentSong().measureLines[Math.floor(beatTimestamp / 1000)].includes(beatTimestamp);
                        this.playedBeats[beatTimestamp] = true;
                        this.audioPlayer.playBeat(beatTimestamp / 1000 - currentTime, newMeasure);
                    }
                });
            }
        });
    ...
    playNote(note) {
        ...
        if (getMidiHandler().isOutputActive()) {
            ...
            this.audioPlayer.playCompleteNote(currentTime, note, this.playbackSpeed, this.getNoteVolume(note), isAnyTrackPlayalong());
        }
    }
    getNoteVolume(note) {
