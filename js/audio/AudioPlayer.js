...
import { createContinuousAudioNote, createCompleteAudioNote } from "./AudioNote.js";
...
export class AudioPlayer {
    constructor() {
        ...
        // make sure we have some defaults loaded
        SoundfontLoader.loadInstrument("acoustic_grand_piano", this.soundfontName);
        SoundfontLoader.loadInstrument("honkytonk_piano", this.soundfontName);
    }
    ...
    createContinuousNote(noteNumber, volume, instrument) {
        ...
        let audioNote = createContinuousAudioNote(this.context, getBufferForNote(this.soundfontName, instrument, noteNumber), volume / 100);
    ...
    playCompleteNote(currentTime, note, playbackSpeed, volume, isPlayAlong) {
        const buffer = getBufferForNote(this.soundfontName, note.instrument, note.noteNumber);

        let audioNote = createCompleteAudioNote(note, currentTime, playbackSpeed, volume, isPlayAlong, this.context, buffer);
    ...
    loadMetronomeSounds() {
        ...
        request2.open("GET", "./js/audio/metronome/2.wav");
    ...
    async loadInstrumentsForSong(currentSong) {
        ...
        let neededInstruments = instrumentsOfSong
            ...
            .map((instrument) => SoundfontLoader.loadInstrument(instrument, this.soundfontName));
        if (instrumentsOfSong.includes("percussion")) {
            neededInstruments.push(SoundfontLoader.loadInstrument("percussion", "FluidR3_GM"));
        }
